        ...
        <div className="flex min-w-0 items-center gap-2">
          <button
            ...
            className={cn(
              ...
              opensPreview && thumbnails.enabled ? 'cursor-pointer hover:opacity-90' : 'cursor-default',
            )}
            disabled={!opensPreview || !thumbnails.enabled}
            onClick={() => {
              if (opensPreview && thumbnails.enabled) setSelectedItem(item);
            }}
            aria-label={opensPreview && thumbnails.enabled ? `Preview ${displayName}` : undefined}
            aria-hidden={item.isParent ? true : undefined}
            tabIndex={item.isParent ? -1 : undefined}
          >
            <ItemThumbnail
              item={item}
